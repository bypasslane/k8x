...
kubectl get namespace | awk -F " " '{print $1}' | tail -n+2 | while read NAMESPACE; do
    ...
    kubectl get deployment --namespace=${NAMESPACE} | awk -F " " '{print $1}' | tail -n+2 | while read DEPLOYMENT; do
        if ! [[ ${DEPLOYMENT} =~  "No resources found." ]]; then
            ...
            kubectl get deployment ${DEPLOYMENT} --namespace=${NAMESPACE} -oyaml --export=true | yq r -j - | jq -S 'del(..|.metadata?)|del(.spec.replicas)' | yq r - > ${DEPLOYMENT}.deployment.yaml  &
        fi
    done
    wait
    echo -e "\t Configmaps:"
    kubectl get configmap --namespace=${NAMESPACE} | awk -F " " '{print $1}' | tail -n+2 | while read CONFIG_MAP; do
        if ! [[ ${CONFIG_MAP} =~  "No resources found." ]]; then
            ...
            kubectl get configmap ${CONFIG_MAP} --namespace=${NAMESPACE} -oyaml --export=true | yq r -j - | jq -S 'del(..|.metadata?)' | yq r - > ${CONFIG_MAP}.configmap.yaml &
        fi
    done
    wait
    echo -e "\t Services:"
    kubectl get service --namespace=${NAMESPACE} | awk -F " " '{print $1}' | tail -n+2 | while read SERVICE; do
        if ! [[ ${SERVICE} =~  "No resources found." ]]; then
            ...
            kubectl get service ${SERVICE} --namespace=${NAMESPACE} -oyaml --export=true | yq r -j - | jq -S 'del(..|.metadata?)' | yq r - > ${SERVICE}.service.yaml &
        fi
    done
    wait
    echo -e "\t Ingresses:"
    kubectl get ingress --namespace=${NAMESPACE} | awk -F " " '{print $1}' | tail -n+2 | while read INGRESS; do
        if ! [[ ${INGRESS} =~  "No resources found." ]]; then
            ...
            kubectl get ingress ${INGRESS} --namespace=${NAMESPACE} -oyaml --export=true | yq r -j - | jq -S 'del(..|.metadata?)|del(.spec.rules[0]?.host?)' | yq r - > ${INGRESS}.ingress.yaml &
        fi
    done
    wait
    cd ..
